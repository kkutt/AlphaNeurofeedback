...
import os, traceback, time, numpy, random, threading
...
class Main(LatentModule):
    ...
    def __init__(self):
        ...
        self.punktySkalarKolor = (0.5, 0.5, 0.5, 1.0)
    ...
    def run(self):
        ...
        #FIXME: wczytywanie plików z zapisem sesji -- w pierwszym wierszu mogą być:
        ...
        # skalar ilości punktów na podstawie kalibZero i kalib10Proc
        tempZero = self.kalibZero / 0.2
        if tempZero < 0 and tempZero >= -1:
            tempZero = 1.0
        elif tempZero < -1:
            tempZero = 1/-tempZero
        self.punktacjaSkalar = (self.kalib10Proc / 0.1) * tempZero
        self.punktacjaSkalar = round(self.punktacjaSkalar, 1)
        
        # rysujemy skalar
        self._engine.direct.gui.OnscreenText.\
        OnscreenText(text = str(self.punktacjaSkalar) + "x",
                     pos = (1.0, self.poziomPunkty),
                     scale = self.punktyRozmNormalny,
                     font = self.punktyCzcionka,
                     mayChange = False,
                     fg = self.punktySkalarKolor)
        ...
        while blok <= self.iloscBlokow:
            ...
            while time.time() < koniecCzasu:
                ...
                if self.warunek == "F":   ## FAKE FEEDBACK
                    ...
                    # FIXME: jak działa kalibracja w FAKE?
                    poziom = self.poziomZero
                    pass
                ...
                if poziom < self.poziomZero - self.pasekMinDlugosc:
                    ...
                    poziom = self.poziomZero - self.pasekMinDlugosc
    ...
    def perform_calibration(self):
        ...
        if self.wcisnietyKlawisz == "enter":
            ...
            kalibMin = self.poziomyFINAL[10]  # 10 najmniejsza wartość (odrzucamy szumy)
            kalibMax = self.poziomyFINAL[-10]  # 10 największa wartość
            print("Kalibracja: MIN=" + str(kalibMin) + 
                  "  Mean=" + str(kalibMean) + 
                  "  MAX=" + str(kalibMax))
            
            ## MIN = 10% od dołu ekranu
            ## MAX = 50% od dołu ekranu
            ## 10% wysokości = (MAX-MIN)/4
            self.kalib10Proc = (kalibMax-kalibMin)/4
            ## poziom zero (narysowany na ekranie w 20% wysokości ekranu):
            self.kalibZero = kalibMin + self.kalib10Proc
            
            #zapisujemy wyliczone wartości do pliku
            self.write_to_subject_file("# kalibMin = " + str(kalibMin) +
                               "\n# kalibMean = " + str(kalibMean) + 
                               "\n# kalibMax = " + str(kalibMax) + 
    ...
    def study_summary(self):
        ...
        self.write_to_subject_file("### Skuteczność:\n" + 
                           ...
                           "# punktacjaSkalar = " + str(self.punktacjaSkalar) + "\n" +
        ...
        if self.nrSesji > 1 and self.punktacja > punktacjaMax:
            ...
            self.write('To już koniec. Dziękujemy za udział w treningu.\n\n' +
                       'Zapraszamy do udziału w kolejnych treningach i ' +
    ...
    def write_to_session_file(self, blok):
        ...
        nazwaPliku = self.folderRejestry + "\\" + self.imieNazwisko + "." + \
                     str(self.nrSesji) + "." + str(blok) + ".alfa"  
    ...
    def skaluj_poziom(self, poziom):
        return (self.poziomAKTUALNY - self.kalibZero) / \
    ...
    def __kule_runner(self):
        ...
        while not self.kuleStopped:
            ...
            for k in self.kuleLista:
                if abs(k.pozPoziom) - self.kuleRuch < 0:
                    # kuleczka przekracza środek ekranu - kasujemy
                    k.obiekt.destroy()
                    ...
                    if abs(k.pozPoziom) - self.kulaRozmiar - \
                       ...
                       self.pasekMinDlugosc:
                        ...
                        self.punktacja += self.punktacjaSkalar * k.punkty
                        ...
                        self.punktyZmianaObiekt.setText("+" + str(self.punktacjaSkalar * k.punkty))
    ...
    class Kula():
        def __init__(self, obiekt, rysunek, pozPion, pozPoziom, punkty):
            self.obiekt = obiekt
